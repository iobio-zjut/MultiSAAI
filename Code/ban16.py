...
class BANLayer(nn.Module):
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='ReLU', dropout=0.2, k=3):
        ...
        # self.bn = nn.BatchNorm1d(h_dim,track_running_stats=False)
        # self.bn = nn.InstanceNorm1d(h_dim)
    ...
    def forward(self, v, q, softmax=False):
        ...
        if self.h_out <= self.c:
            ...
            # print("q_", q_.shape)
            #([1, 225, 1536])
            # print("self.h_mat", self.h_mat.shape)
            #([1, 2, 1, 1536])
        ...
        # print("logits_0", logits.shape)
        for i in range(1, self.h_out):
            logits_i = self.attention_pooling(v_, q_, att_maps[:, i, :, :])
            # print("i=",i)
            # print("logits_i", logits_i.shape)
            logits += logits_i
